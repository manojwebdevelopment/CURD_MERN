...
function Update() {
  ...
  useEffect(() => {
    const userfetch = async () => {
      try {
        const response = await axios.get(`https://curd-mern-1.onrender.com/user/${id}`); // Adjusted to GET for fetching
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.put(`https://curd-mern-1.onrender.com/update/${id}`, user); // Pass the ID in the URL
